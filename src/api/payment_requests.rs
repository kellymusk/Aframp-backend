...
pub async fn list(
    ...
    let rows = payment_requests::payment_requests_by_merchant(&state.db, merchant_id, limit + 1, before)
        .await
        .map_err(internal)?;

    let next_cursor = if rows.len() > limit as usize {
        rows.get(limit as usize).map(|r| r.created_at.to_rfc3339())
    } else {
        None
    };
    ...
    Ok(Json(PaymentRequestListResponse {
        payment_requests: rows.iter().map(row_to_view).collect(),
        next_cursor,
    }))
}
...
fn build_sep7_uri(address: &str, amount_stroops: i64, asset: &str, memo: &str) -> Option<String> {
    ...
    let encoded_memo = urlencoding::encode(memo);
    Some(format!(
        "web+stellar:pay?destination={address}&amount={amount}&memo={encoded_memo}&memo_type=MEMO_TEXT"
    ))
}
